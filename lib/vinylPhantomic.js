...
module.exports = function vinylPhantomic(options, callback) {
  ...
  var types = _.extend({
    js: {
      ...
      concat: function() {
        ...
        .pipe(gulpConcat('bundle.js', {newLine: ';\n'}))
        .pipe(gulpSourceMap.write())
        .pipe(output);
        return es.duplex(input, output);
      },
      onComplete: function(data) {
        // Yea, this is really ugly.
        // FIXME
        var jsSourceMaps = sourceMapper.extract(data.toString());
        phantomOutput
        .pipe(sourceMapper.stream(jsSourceMaps.map))
        .pipe(output);
      }
    },
    html: {
      filter: gulpFilter('**/*.html'),
      concat: function() { return gulpConcat('bundle.html'); }
    },
    css: {
      filter: gulpFilter('**/*.css'),
      concat: function() { return gulpConcat('bundle.css'); },
      onComplete: function(data) {
        if(data.length >= 1)
  ...
  function startServer() {
    if (_.some(bundles, _.isNull)) return;
